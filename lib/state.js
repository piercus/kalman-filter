...
class State {
	...
	* @param {Array.<Number>} selectedIndexes list of dimension to extract,  (M < N <=> selectedIndexes.length < this.mean.length)
	* @returns {State} subState in subspace, with subState.mean.length === selectedIndexes.length
	*/
	...
	*/
	detailedMahalanobis({kf, observation, selectedIndexes}) {
		...
		if (Array.isArray(selectedIndexes)) {
			projectedState = projectedState.subState(selectedIndexes);
			correctlySizedObservation = selectedIndexes.map(i => correctlySizedObservation[i]);
		}
