...
const useModel = function({options,registeredModels, name}) {
	if (registeredModels.hasOwnProperty(name) {
		return Object.assign({}, options, registeredModels[name])
	}
};

const checkDimensions({options}) {
...
//Build stateProjection if doesn't exist
const build = function({options}) {
	if (options.observation.observedProjection && options.observation.stateProjection) {
...
const fill = function(options) {
	...
	const withDimensionOptions = checkDimensions({options: modelsOptions});
	build(withDimensionOptions);
