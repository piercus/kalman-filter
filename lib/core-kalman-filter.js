...
class CoreKalmanFilter {
	...
	*/
	...
	predict({previousCorrected} = {}) {
		...
		const getValueOpts = {previousCorrected, index: previousCorrected.index};
		const d = this.getValue(this.dynamic.transition, getValueOpts);
		...
		const dynCov = this.getValue(this.dynamic.covariance, getValueOpts);
	...
	*/
	...
	getGain({predicted, stateProjection}) {
		const getValueOpts = {predicted, index: predicted.index};
		stateProjection = stateProjection || this.getValue(this.observation.stateProjection, getValueOpts);
		const obsCovariance = this.getValue(this.observation.covariance, getValueOpts);
	...
	*/
	...
	correct({predicted, observation}) {
		...
		const stateProj = this.getValue(this.observation.stateProjection, getValueOpts);
