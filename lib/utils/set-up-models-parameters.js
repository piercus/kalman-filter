...
const modelCollections = require('../lib/model-collections.js');
/**
...
module.exports = function ({observation, dynamic}) {
	if (typeof(observation.name) === 'string') {
		observation = modelCollections.buildObservation(observation);
	}
	if (typeof(dynamic.name) === 'string') {
		dynamic = modelCollections.buildObservation(dynamic);
	}
